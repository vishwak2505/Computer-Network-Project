...
NS_LOG_COMPONENT_DEFINE ("RIP-Ipv4");
...
int main (int argc, char **argv)
{
  ...
  if (verbose)
  {
      LogComponentEnable ("RIP-Ipv4", LOG_LEVEL_INFO);/*
  ...
  for(int i=0;i<nr+nh;i++)
  {
    ...
    Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("RIP-IPv4.txt", std::ios::out);
  ...
  {    
      ...
      staticRouting = Ipv4RoutingHelper::GetRouting <Ipv4StaticRouting> (hosts.Get(s)->GetObject<Ipv4> ()->GetRoutingProtocol ());
      staticRouting->SetDefaultRoute (nethosts[k].GetAddress(0), 1 );
      staticRouting = Ipv4RoutingHelper::GetRouting <Ipv4StaticRouting> (hosts.Get(c)->GetObject<Ipv4> ()->GetRoutingProtocol ());
      staticRouting->SetDefaultRoute (nethosts[k2].GetAddress(0), 1 );
   ...
   p2p.EnableAsciiAll (ascii.CreateFileStream ("RIP-IPv4.tr"));
...
//   }
  ...
  anim= new AnimationInterface("Rip-IPv4.xml");
  ...
  Simulator::Run ();
  Simulator::Destroy ();
  flowMonitor->SerializeToXmlFile("Rip-IPv4-flow.xml", true, true);  
